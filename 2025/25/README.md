...
We have already distributed all the gifts! Back at the workshop, preparations for next year are already beginning. A genius elf is creating a **magical programming language** 🪄 that will help streamline the delivery of gifts to children in 2025.

Programs always start with the value `0`, and the language is a string where each character represents an instruction:

- `>`: Moves to the next instruction (effectively does nothing on its own).
...
You need to **return** the **final value** of the program after executing all the instructions.
