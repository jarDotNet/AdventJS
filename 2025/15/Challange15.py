...
def draw_table_original(data: list[dict[str, str | int]], sortBy: str) -> str:
  CORNER = "+"
  HYPHEN = "-"
  VERTICAL_BAR = "|"
  ...
  h_separator = CORNER
  header = VERTICAL_BAR
  for index, prop in enumerate(max_lengths):
    h_separator += f"{(HYPHEN * (max_lengths[prop]+2))}{CORNER}" 
    header +=  f" {LETTERS[index]} {' ' * (max_lengths[prop]-2)} {VERTICAL_BAR}" 

  rows = []
  for dictionary in sorted_data:
    row = VERTICAL_BAR

    for _, prop in enumerate(dictionary):
      item_value = dictionary[prop]
      item_length = len(str(item_value))
      max_length = max_lengths[prop]
      row += f" {item_value}{' ' * (max_length - item_length)} {VERTICAL_BAR}"
